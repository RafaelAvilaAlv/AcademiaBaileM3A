...
public class ControladorHorario {
    ...
    public void iniciarControl() {
        try{
         vista.getTxtcodigo().setText(String.valueOf(modelo.CargarCod()));
        vista.getBtnCrear().addActionListener(l -> abrirDialogCrear());
        vista.getBtnGuardar().addActionListener(l -> crearEditarHorario());
...
//        buscarHorario();
    ...
    public void cargarTablaDeHorario() {
        ...
        horarios.stream().forEach(p -> {
            String[] datos = {String.valueOf(p.getCod_horario()), p.getHora_ini(), p.getHora_fin(), p.getDia()};
            tabla.addRow(datos);
        });
    }
   ...
   /* public void imprimir() {
    ...
    private void crearEditarHorario() {
        if ("Crear nuevo horario".equals(vista.getjDlgHorario().getName())) {

            //INSERTAR
            if (validarDatos()) {
                modelo.setCod_horario(Integer.parseInt(vista.getTxtcodigo().getText()));
                modelo.setDia(vista.getComboDia().getSelectedItem().toString());
                modelo.setHora_ini(vista.getComboHoraInicio().getSelectedItem().toString() + ":" + vista.getComboMinutosInicio().getSelectedItem().toString());
                modelo.setHora_fin(vista.getComboHoraFin().getSelectedItem().toString() + ":" + vista.getComboMinutosFin().getSelectedItem().toString());
            ...
            if (validarDatos()) {
                ...
                modelo.setCod_horario(codigoHorario);
                modelo.setDia(vista.getComboDia().getSelectedItem().toString());
                modelo.setHora_ini(vista.getComboHoraInicio().getSelectedItem().toString() + ":" + vista.getComboMinutosInicio().getSelectedItem().toString());
                modelo.setHora_fin(vista.getComboHoraFin().getSelectedItem().toString() + ":" + vista.getComboMinutosFin().getSelectedItem().toString());
    ...
    public void cargarDatosHorarioEnTXT() {
        ...
        if (fila == -1) {
            ...
            listap.stream().forEach(h -> {
                ...
                if (h.getCod_horario() == Integer.parseInt(vista.getTblHorario().getValueAt(fila, 0).toString())) {

                    codigoHorario = h.getCod_horario();
                    for (int j = 0; j < vista.getComboDia().getItemCount(); j++) {
                        if (vista.getComboDia().getItemAt(j).equalsIgnoreCase(h.getDia())) {
                    ...
                    for (int j = 0; j < vista.getComboHoraInicio().getItemCount(); j++) {
                        if (vista.getComboHoraInicio().getItemAt(j).equalsIgnoreCase(h.getHora_ini().substring(0, 2))) {
                    ...
                    for (int j = 0; j < vista.getComboMinutosInicio().getItemCount(); j++) {
                        if (vista.getComboMinutosInicio().getItemAt(j).equalsIgnoreCase(h.getHora_ini().substring(3, 5))) {
                    ...
                    for (int j = 0; j < vista.getComboHoraFin().getItemCount(); j++) {
                        if (vista.getComboHoraFin().getItemAt(j).equalsIgnoreCase(h.getHora_fin().substring(0, 2))) {
                    ...
                    for (int j = 0; j < vista.getComboMinutosFin().getItemCount(); j++) {
                        if (vista.getComboMinutosFin().getItemAt(j).equalsIgnoreCase(h.getHora_fin().substring(3, 5))) {
